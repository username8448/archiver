...
5. Call `POST /api/profile/preview` from the UI and confirm the response has real `profile.posts[].shortcode` values, `preview_url` fields when Instagram provides them, and `type` values limited to `photo`, `video`, `carousel`, or `unknown`.
...
7. Click `Загрузить ещё`; it must call `/api/profile/preview` with a larger limit and `force_refresh: true`.
8. Select posts and create a job; `mode: "selected"` must send real shortcodes.
9. Test `last-n` and `meta-only`; requests must send real target/mode/limit/options, and `meta-only` must send `media: false`.
10. Confirm photo/carousel jobs use `gallery-dl`, video/reel jobs use `yt-dlp`, and generated metadata records the downloader name.
